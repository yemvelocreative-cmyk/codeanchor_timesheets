// Timekeeper — Timesheet (clean) [DROP-IN SIMPLIFIED]
(function () {
  'use strict';
    ...
    fetchJSON(tkTicketsUrl(clientId))
      .then(items => {
        ...
        if (initialValue) {
          ...
          if (!selectEl.value) {
            // if initial not in list, synthesize a selected option
            selectEl.appendChild(new Option('Selected Ticket (' + initialValue + ')', String(initialValue), true, true));
          }
        }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (addForm) {
      ...
      bindDeptTaskFilter(document.getElementById('department_id'), document.getElementById('task_category_id'));
      ...
      if (clientSel && ticketSel) {
        ...
        const onClientChange = function () {
          ...
          if (cid) { populateTicketSelect(ticketSel, cid, null); }
          else {
            while (ticketSel.options.length) ticketSel.remove(0);
            ticketSel.appendChild(new Option('Select a ticket…', ''));
            if (window.jQuery && jQuery.fn && jQuery.fn.select2) {
    ...
    document.querySelectorAll('form.tk-row-edit').forEach(form => {
      ...
      if (billChk && billInp) {
        function setBill() {
          ...
          else if (!billInp.value && spent && spent.value) billInp.value = spent.value;
        }
      ...
      if (slaChk && slaInp) {
        function setSla() {
          ...
          else if (!slaInp.value && spent && spent.value) slaInp.value = spent.value;
        }
      ...
      if (rowClient && rowTicket) {
        ...
        const onRowClientChange = function () {
          ...
          if (cid) { populateTicketSelect(rowTicket, cid, null); }
          else {
            while (rowTicket.options.length) rowTicket.remove(0);
            rowTicket.appendChild(new Option('Select a ticket…', ''));
            if (window.jQuery && jQuery.fn && jQuery.fn.select2) {
